"""Files command - enumerate files in a directory tree."""
...
from aux.plans import FilesPlan, parse_plan


CAPABILITY: dict = {
    "name": "files",
    "description": "Enumerate files by name, glob, or extension in a directory tree.",
    "category": "read",
    "intent_signals": [
        "list files in a directory",
        "find files matching a glob pattern",
        "surface discovery before reading content",
    ],
    "requires": ["root"],
    "optional_deps": [],
    "compose_with": ["search", "find"],
    "mutates": False,
    "schema_cmd": "aux files --schema",
}


def register_parser(subparsers: argparse._SubParsersAction) -> None:
    """Register the files subcommand."""
    parser = subparsers.add_parser(
        "files",
        help="Enumerate files by name/glob using fd",
        description="""
Enumerate files matching criteria using fd.

Simple usage:
  aux files --root /path [--glob GLOB] [--type file|directory]
...
""",
    ...
    parser.set_defaults(func=cmd_files)
...
def cmd_files(args: argparse.Namespace) -> int:
    ...
    if args.plan:
        try:
            plan = parse_plan(args.plan, FilesPlan)
