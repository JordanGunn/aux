...
Usage (run):
  skill.sh run <symbol> --root <path> [options]
...
Examples:
  skill.sh run DataProcessor --root ./src --glob "**/*.py"
  skill.sh run process_batch --root /path --glob "**/*.py" --no-definitions
  echo '{"root":"/path","symbol":"DataProcessor","globs":["**/*.py"]}' | skill.sh run --stdin
...
cmd_validate() {
    ...
    if ! python -c "import tree_sitter" 2>/dev/null; then
        echo "warn: tree-sitter not installed (definition tagging unavailable). Install with: pip install 'aux-skills[query]'" >&2
    fi
