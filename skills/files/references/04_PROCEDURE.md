...
- Parse the user's `/files <prompt>` invocation
...
# or directly: aux files --schema
```

**Simple mode** (options as flags):

```bash
aux files --root /path --glob "*.py" --type file
```
...
Run `aux files --help` for the complete, authoritative option list:
