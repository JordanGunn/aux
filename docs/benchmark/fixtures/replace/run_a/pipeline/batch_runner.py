"""
pipeline.batch_runner — run EventProcessor over a list of record batches.

BatchRunner orchestrates multiple EventProcessor instances, one per partition,
...
class BatchRunner:
    """Drive a EventProcessor across partitioned record batches.

    BatchRunner accepts a preconfigured EventProcessor and a list of batches.
    Each batch is processed in a thread pool and results are merged in
    completion order.

    Args:
        processor: A EventProcessor instance shared across all batch workers.
        max_workers: Thread pool size (default: 4).
    """

    def __init__(self, processor: EventProcessor, max_workers: int = 4) -> None:
        self.processor: EventProcessor = processor
        self.max_workers = max_workers

    def run(self, batches: list[list[dict]]) -> Iterator[dict]:
        """Process all batches via the EventProcessor and yield results.
...
def run_batches(
    ...
    """Convenience wrapper: run all batches through EventProcessor and collect results.

    Args:
        processor: EventProcessor to use.
