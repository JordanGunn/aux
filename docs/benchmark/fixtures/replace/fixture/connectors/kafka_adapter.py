"""
connectors.kafka_adapter — Kafka consumer adapter for DataProcessor pipelines.

KafkaAdapter subscribes to one or more topics and feeds raw messages into a
DataProcessor, yielding processed records to the caller.
"""
...
class KafkaAdapter:
    """Connect a Kafka topic subscription to a DataProcessor.

    KafkaAdapter handles deserialization, offset management, and error
    recovery before handing records off to DataProcessor for validation and
    transformation.

    Args:
        topics: List of Kafka topic names to subscribe to.
        processor: DataProcessor instance that receives each decoded message.
    ...
    def consume(self, max_messages: int = 100) -> Iterator[dict]:
        """Poll Kafka and yield records processed by DataProcessor.

        Args:
            max_messages: Maximum number of raw messages to consume per call.

        Yields:
            Validated, transformed records from DataProcessor.
        """
