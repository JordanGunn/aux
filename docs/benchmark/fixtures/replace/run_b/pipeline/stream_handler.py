"""
pipeline.stream_handler — real-time record stream handler backed by EventProcessor.

StreamHandler wraps a EventProcessor and exposes a blocking iterator interface
...
class StreamHandler:
    """Push records through a EventProcessor as they arrive.
    ...
    Args:
        processor: EventProcessor used for validation and transformation.
    ...
    def subscribe(self, callback: Callable[[dict], None]) -> None:
        """Register a callback to receive EventProcessor output records."""
        self._callbacks.append(callback)

    def handle(self, records: Iterable[dict]) -> None:
        """Process *records* through EventProcessor and fan out to subscribers.
