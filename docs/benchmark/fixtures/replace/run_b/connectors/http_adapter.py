"""
connectors.http_adapter — HTTP polling adapter for EventProcessor pipelines.

HttpAdapter periodically fetches records from a REST endpoint and passes
them through a EventProcessor before forwarding to downstream consumers.
"""
...
class HttpAdapter:
    """Poll an HTTP endpoint and feed results into EventProcessor.

    HttpAdapter calls a JSON REST endpoint on each :meth:`fetch` invocation,
    decodes the response body, and delegates record processing to EventProcessor.

    Args:
        url: Fully-qualified HTTP URL to poll.
        processor: EventProcessor used to validate and transform each record.
    ...
    def fetch(self) -> Iterator[dict]:
        """Fetch one page of records and yield EventProcessor output.

        Yields:
            Validated, transformed records produced by EventProcessor.
        """
        raw = self._get()
